...
@interface KYLCamera ()
{
    ...
    int m_bRemoteRecordVideoShowing;//远程回放视频是否已经开始显示
    ...
    int m_iLastVideoStatusBeforeInBackground;//记录摄像机在手机锁屏或者进入后台前的视频状态
    BOOL m_bFindIFrame;
    BOOL m_bCameraReconnectStarted;
    BOOL m_bSucceedGetCameraSoftVersionInfo;
    int m_bIsPrepareForSnapPicture;//是否要抓拍，抓拍的时候，进行yuv转rgb24
    ...
    int m_bIsSendingTalkDataToDevice;//如果正在发送对讲数据，则要停止手机声音播放（m_bIsSendingTalkDataToDevice=1），当发送对讲数据停止，如果监听开启则播放声音
}
...
- (void) clearAllResources
{
    ...
    self.m_bIsAdmin = 0;
    self.m_iIndexForTableView = 0;//记录在设备列表中的位置，第几行
    self.m_iIndexForImageView = 0;//记录在那个播放窗口显示
...
- (void) initData
{
    ...
    m_bIsPrepareForSnapPicture = 1;//是否要抓拍，抓拍的时候，进行yuv转rgb24
...
{
    DebugLog(@"设置 camera token did=%@ , m_sCameraToken=%@",self.m_sDID, sToken);
...
+ (int) initSDK
{
    ...
    ST_InitStr stInitStr[2];
    memset(&stInitStr, 0, sizeof(stInitStr));
    
    strcpy(stInitStr[0].chInitStr, "EFGBFFBJKFJKGGJBENHHFOEHGPJJDJNBGNFLEIDOEFJFPJPBHNBKGPPGHAOAMMLOAJNDPPCCLDNLFNGNMHNMNFBOMM");
    
    strcpy(stInitStr[0].chPrefix, "VIEW");
...
- (int ) connect
{
    //@synchronized(self)
    {
        ...
        DebugLog(@"the connect end result = %d, DID = %@, username=%@, password=%@", m_iRet, self.m_sDID, self.m_sUsername, self.m_sPassword);
...
- (NSMutableArray *)getCameraSupportResolutionList
{
    ...
    for(int i = 0; i<7; i++)
    {
        ...
        //DebugLog(@"%@ 遍历分辨率  p[%d]=%d" ,_m_sDID,i,(int)nTempValue);
        if (nTempValue == -1)
        {
            //DebugLog(@"不支持该分辨率，pVideoParams[%d]=%d",i,0xFF);
            continue;
        }
        //DebugLog(@" 遍历所有支持的分辨率  p[%d]=%d",i,(int)nTempValue);
...
- (int) getTheAVCodeParameter
{
    ...
    DebugLog(@"\n连线成功后，获取视频信息, did=%@ nVideoCodecID=%d, nAudioCodecID=%d, m_sNVideoParameter=%@, nAudioParameter=%d, deviceType=%d, nvrChannelNum=%d \n", _m_sDID,_nVideoCodecID,_nAudioCodecID,_m_sNVideoParameter,_nAudioParameter,_m_nDeviceType,_m_nNVRChannelNum);
...
{
    int nRet = 0;
    if (nil == self.m_listNVRCameras && DEVICE_TYPE_NVR == _m_nDeviceType) {//如果是NVR,则创建对应通道数量的KYLCamera 对象
...
- (int) startLocalRecord
{
    ...
    if(NULL == m_pMp4videoRecordToolHandle)
    {
        ...
        if(nRet < 0)
        {
            ...
            //打开录像文件，打开后在播放视频时，如果开启录像每次写入一帧数据
            ...
            NSLog(@"---》调用打开录像文件函数SEMP4_OpenMp4File() nRet=%d",nRet);
            if(nRet>= 0)
            {
...
- (int) addOneVideoFrameIntoLocalRecordFile:(char *) pData dataSize:(NSInteger) nDataSize deviceTimeTamp:(NSInteger) nDeviceTimeStamp localTimeTamp:(NSInteger) nLocalTimeTamp isIFrame:(int) bIsIFrame
{
    ...
    if(m_bLocalRecording && m_pMp4videoRecordToolHandle)
    {
        ...
        //NSLog(@"本地录像，写入一帧视频数据*** nRet =%d",nRet);
    }
    return nRet;
}
...
- (int) startGatherTheTalkAudioThread
{
    ...
    {
        DebugLog(@"启动对讲音频采集线程失败，对讲线程正在运行");
        return 1;
    }
    ...
    if(0 == m_pTalkAudioBuf->Create(KYL_ABUF_SIZE))
    {
        ...
        DebugLog(@"启动对讲音频采集线程失败，创建对讲循环缓冲区失败");
        return -1;
    }
    ...
    if (NULL == m_pPCMRecorder) {
        m_bTalkThreadRuning = 0;
        DebugLog(@"启动对讲音频采集线程失败，创建录音实例失败");
        return -2;
    }
...
- (int) stopTalk
{
    ...
    [self stopSendTalkData]; //2016-03-18
    return nRet;
}
...
- (int) getAllRemoteAllTypeRecordFilesByDayWithYearMonthDay:(int) nYearMonthDay
{
    ...
    stReq.nBeginNoOfThisTime = nBeginNoOfThisTime;
    stReq.nEndNoOfThisTime = nEndNoofThisTime;
    NSLog(@"KYLCamera::getAllRemoteAllTypeRecordFilesByDayWithYearMonthDay() 查询的日期：%d",nYearMonthDay);
    // send the get camera's params command to device
    nRet = SEP2P_SendMsg((CHAR *)[self.m_sDID UTF8String], SEP2P_MSG_GET_REMOTE_REC_FILE_BY_DAY_REQ, (CHAR *)&stReq, sizeof(stReq));
    return nRet;
}
...
- (void) didReceivedEventCallBack:(CHAR *) pDID  eventData:(CHAR*  ) pEventData  dataSize:(UINT32 ) nEventDataSize eventType:(UINT32) nEventType
{
    @synchronized(self)
    {
        NSLog(@"---->IPC收到事件信息，didReceivedEventCallBack DID=%s, eventype=%d datasize=%d",pDID, nEventType,nEventDataSize);
        NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
        NSString *stempDid = [[NSString alloc] initWithCString:pDID encoding:NSUTF8StringEncoding];
...
- (int) dealWithTheLiveAudioStreamCallbackResult:(CHAR *) _sStreamData  dataSize:(UINT32) nSize
{
    ...
    if(0==m_bIsSendingTalkDataToDevice) { //没有发送对讲数据时，写入监听数据
        //NSLog(@"dealWithTheLiveAudioStreamCallbackResult, nSize=%d", nSize);
        nRet = [self AddOneFrameIntoAudioBuf:(char *) _sStreamData dataSize:nSize];
        if(nRet == 0) [self ResetTheAudioBuf];
    }else [self ResetTheAudioBuf];
...
- (int) dealWithTheSDCardVideoStreamCallbackResult:(CHAR *) _sStreamData  dataSize:(UINT32) nSize
{
    ...
    {//only deal the same playback id
        ...
        if(pStreamHead->nCodecID==AV_CODECID_VIDEO_H264 )
        {
            ...
            if (nDataLength == 0) {//说明SDCard录像回放完成
                NSLog(@"\n\n接收数据回调函数， SDCard录像回放数据接收完毕-----");
            }
            if(!m_bFindIFrame){
                if(pStreamHead->nParameter == VIDEO_FRAME_FLAG_I) m_bFindIFrame = true;
                else {
...
- (int) dealTheReceivedMsg:(void *) _sData
{
	...
	switch(pMsgInfo->nMsgType)
	{
        ...
        case SEP2P_MSG_STOP_PLAY_REC_FILE_RESP:// the response on stop remote record play .
        {
            ...
            break;
        case SEP2P_MSG_GET_IPUSH_INFO_RESP:// 获取设备是否允许推送告警信息的状态 .
        {
            ...
            break;
        case SEP2P_MSG_SET_IPUSH_INFO_RESP:// 设置设备开启，关闭推送告警信息，设置结果.
        {
    ...
    @synchronized(self)
    {
        switch (nStatus) {
            ...
            case CONNECT_STATUS_CONNECTED:
                {
                    //如果当前视频播放线程没有停止，则重新请求视频流，如果音频，对讲没有停止，则重新请求音频流
                    ...
                    //发送请求获取设备版本信息，只获取一次，如果获取过，则不去获取
...
- (void) reRequestTheVideoStream
{
    if (m_bVideoPlayThreadRuning)
    {
        if (m_bVideoPlayThreadRuning)
        {
            ...
            {
                //通知重新请求视频流，成功。
                NSLog(@"----->>reSend start the video stream 重新请求视频流，成功");
                // send notification
...
//    NSLog(@"%s MSG_GET_SDCARD_REC_PARAM_RESP, bRecordCoverInSDCard=%d,  nRecordTimeLen=%d, reserve1=%d, bRecordTime=%d, reserve2=%s, nSDCardStatus=%d,   nSDTotalSpace=%d,  nSDFreeSpace=%d\n", [self.m_sDID UTF8String], pResp->bRecordCoverInSDCard,pResp->nRecordTimeLen, pResp->reserve1, pResp->bRecordTime, pResp->reserve2, pResp->nSDCardStatus, pResp->nSDTotalSpace, pResp->nSDFreeSpace);
    int bRecordCoverInSDCard = pResp->bRecordCoverInSDCard; //±Ì æ¬ºœÒ∏≤∏« 0->±Ì æ≤ª≥‰–Ì 1->±Ì æ≥‰–Ì
    int nRecordTimeLen = pResp->nRecordTimeLen;		 //¬ºœÒ ±≥§, in minutes
    int reserve1 = pResp->reserve1;
    int bRecordTime = pResp->bRecordTime;  //±Ì æ∂® ±¬ºœÒ≥‰–ÌŒª, 0->±Ì æ≤ª≥‰–Ì 1->±Ì æ≥‰–Ì
    NSString *strReserve2 = nil;
    int nSDCardStatus = pResp->nSDCardStatus;		//0: no sdcard; 1:with sdcard
    int nSDTotalSpace = pResp->nSDTotalSpace;	//in MBytes
    int nSDFreeSpace = pResp->nSDFreeSpace;		//in MBytes
...
- (int) dealTheSetIPushInfoResp:(MSG_SET_IPUSH_INFO_RESP *) pResp
{
    ...
    NSString *strReserve =[NSString stringWithCString:pResp->reserve encoding:NSUTF8StringEncoding];
    DebugLog(@"设置设备推送功能 Did=%@, 开启推送告警信息功能 nResult = %d, bEnable=%d reserve=%@",self.m_sDID,nResult, bEnable,strReserve);
    if (bEnable == 0) {
        self.m_bAllowCameraPushMsg = 0;
    }
    else
    ...
    //type = 1 表示是设置推送功能, type = 0 表示是获取推送功能状态
...
- (int) dealTheGetIPushInfoResp:(MSG_GET_IPUSH_INFO_RESP *) pResp
{
    ...
    int bEnable = pResp->bEnable;
    NSString *strReserve =[NSString stringWithCString:pResp->reserve encoding:NSUTF8StringEncoding];
    DebugLog(@"获取设备推送功能 Did=%@, 开启推送告警信息功能 nResult = %d, bEnable=%d reserve=%@",self.m_sDID,nResult, bEnable,strReserve);
...
- (int) setTheUserInfo:(NSString *) sDID admin:(NSString *) sAdmin adminPassword:(NSString *) adminPwd vistor:(NSString *) sVistor vistorPassword:(NSString *) visitorPwd
{
    ...
    NSLog(@"开始修改用户名密码信息，admin=%@, adminPwd=%@, vistor=%@, vistorPwd=%@ --the did=%@",sAdmin,adminPwd,sVistor,visitorPwd,sDID);
    int nRet = -1;
    if (sDID == nil || [sDID length] < 1) {
        return nRet;
    }
...
{
    NSLog(@"开始设置Ftp, ftpServer=%@, name=%@, password=%@, ftpPath=%@, port=%d, mode=%d, uploadTime=%d, the did=%@",sFtpServer,user,pwd,sPath,nPort,nMode,nUptime,sDID);
    if ([self getCameraStatus] != CONNECT_STATUS_CONNECTED )
    {
        return K_Error_Camera_Connected_Failed;
    }
...
- (int) setTheAlarmInfo2:(MSG_SET_ALARM_INFO_REQ *) pReq
{
    ...
    if (nRet == ERR_SEP2P_SUCCESSFUL) {
        NSLog(@"the function: setTheAlarmInfo2() excute succeed，Ret=%d, did=%@",nRet,_m_sDID);
    }else NSLog(@"the function: setTheAlarmInfo2() excute failed，Ret=%d, did=%@",nRet,_m_sDID);
...
- (int) SendAudioRecordThread
{
    ...
    char *AudioBuf= NULL;//编码前的缓冲区
    UCHAR *AudioForEncodeBuf=NULL;//编码后的缓冲区
    ...
    AudioBuf=(char *)malloc(nAudioBufSize);//编码前的缓冲区
    AudioForEncodeBuf=(UCHAR *)malloc(nAudioEncodeSize);//编码后的缓冲区
    ...
    while(m_bTalkThreadRuning)
    {
        ...
        {
            DebugLog(@"对讲发送线程退出....m_bTalkResp!=KYL_Talk_REQUEST_STATUS_SUCCEED");
            [pool1 drain];
            break;
        }
        
        //DebugLog(@"对讲发送线程正在运行....m_bTalkResp=%d",m_bTalkResp);
        if (m_iTalkEncodeType == AV_CODECID_AUDIO_G726) { //g726
            ...
            DebugLog(@"读取到录音数据 nRead=%d",nRead);
            ...
            if(m_pHandleEncode)
            {
                ...
                if (CONNECT_STATUS_CONNECTED == [self getCameraStatus])
                {
                    ...
                    if(nRet > 0 && nAudioEncodeSize > 0)
                    {
                        ...
                        DebugLog(@"发送对讲数据 G726 nRet=%d datasize=%d, time=%d",nRet ,nAudioEncodeSize, (int)curtime);
                        if(nRet >= 0)
                        {
                            //succeed send
                        }
        ...
        { //adpcm
            ...
            if(m_pHandleEncode){
                if (CONNECT_STATUS_CONNECTED == [self getCameraStatus])
                {
                    ...
                    {
                        nRet=SEP2P_SendTalkData([self.m_sDID UTF8String], (CHAR *)AudioForEncodeBuf, nAudioEncodeSize, curtime);
                        //DebugLog(@"发送对讲数据 ADPCM nRet=%d datasize=%d, time=%d",nRet ,nAudioEncodeSize, (int)curtime);
                        if(nRet >= 0)
                        {
                            //succeed send
                        }
            ...
            if(m_pHandleEncode)
            {
                ...
                if (CONNECT_STATUS_CONNECTED == [self getCameraStatus])
                {
                    ...
                    {
                        nRet=SEP2P_SendTalkData([self.m_sDID UTF8String], (CHAR *)AudioForEncodeBuf, nAudioEncodeSize, curtime);
                        DebugLog(@"发送对讲数据 G711 nRet=%d datasize=%d, time=%d",nRet ,nAudioEncodeSize, (int)curtime);
                        if(nRet >= 0)
                        {
                            //succeed send
                        }
            ...
            //DebugLog(@"读取AAC数据 GetStock=%d, KYL_MIN_PCM_AUDIO_SIZE_gAAC=%d",m_pTalkAudioBuf->GetStock(),KYL_MIN_PCM_AUDIO_SIZE_gAAC);
            ...
            if(m_pHandleEncode)
            {
                ...
                if (CONNECT_STATUS_CONNECTED == [self getCameraStatus])
                {
                    ...
                    {
                        nRet=SEP2P_SendTalkData([self.m_sDID UTF8String], (CHAR *)AudioForEncodeBuf, nAudioEncodeSize, curtime);
                        NSLog(@"发送对讲数据 AAC nRet=%d datasize=%d, time=%d",nRet ,nAudioEncodeSize, (int)curtime);
                    }
                }
            }
        }
...
- (int) PlayVideoThread
{
    ...
    while(m_bVideoPlayThreadRuning)
    {
        ...
        int videoLen = 0;
        {
            ...
            if(m_bLocalRecording)
            {
                ...
                if(AV_CODECID_VIDEO_H264 == nTempCodecID){
                    if(!bFirstIFrameWhenRecording) {
                        if(nTempParameter == VIDEO_FRAME_FLAG_I) bFirstIFrameWhenRecording=1;//判断第一帧是否是Ｉ帧
                    }
            ...
            {
                if(!bFirstIFrame) {
                    if(nTempParameter == VIDEO_FRAME_FLAG_I) bFirstIFrame=1;//判断第一帧是否是Ｉ帧
                }
                ...
                // NSLog(@"实时视频解码yuv] pDID=%@, SEVideo_Decode2YUV=%d in_outLen=%lu\n", self.m_sDID, nRet, in_outLen);
                if(nRet>0)
                {
                    if(m_bIsPrepareForSnapPicture == 1)//只有开启了抓拍开关后，才进行解码Rgb24数据
                    {
...
{
    if (m_bAudioPlayThreadRuning) {//当前线程正在运行，直接退出
        NSLog(@"startPlayAudioThread()当前线程正在运行");
        return -2;
    }
                    ...
                    if (m_pAudioPlayer)
                    {
                        ...
                        if(pHandle && pOutBuf)
                        {
                            ...
                            if(nRet > 0 && nOutLen > 0)
                            {
                                ...
                                if (m_bIsSendingTalkDataToDevice == 0 ) {//没有发送对讲数据，则播放声音
...
{
    if (m_bVideoPlayThreadRuning) {//当前线程正在运行，直接退出
        NSLog(@"开启实时视频失败，当前线程正在运行");
        return -2;
    }
    m_bVideoPlayThreadRuning = 1;
    m_bFindIFrame = false;
	...
	dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{//启动线程
        ...
        if (ret >= 0)
        {
            NSLog(@"PlayVideoThread 实时视频线程结束");
        }
		dispatch_async(dispatch_get_main_queue(), ^{ //回到主线程
...
- (int) PlayTheRemoteRecordVideoThread
{
    ...
    while(m_bRemoteRecordVideoPlayThreadRuning)
    {
        ...
        int videoLen = 0;
        {
            ...
            //NSLog(@"SDcard录像回放--sleep时间：%d",(int)nTempDiffTime);
            if(nTempDiffTime > 0 && nTempDiffTime <= 1000){
...
{
    if (m_bRemoteRecordVideoPlayThreadRuning) {//当前线程正在运行，直接退出
        NSLog(@"startPlayRemoteRecordVideoThread()当前线程正在运行");
        return -1;
    }
    int nTempCameraType = [self cameraType];
    int nTempBufSize = KYL_VBUF_SIZE;
...
{
    if (m_bRemoteRecordAudioPlayThreadRuning) {//当前线程正在运行，直接退出
        NSLog(@"startPlayRemoteRecordAuidoThread()当前线程正在运行");
        return -1;
    }
...
//    memcpy(&self.m_pAVParameter,&other.m_pAVParameter,sizeof(AV_PARAMETER));
    ...
    self.m_iIndexForTableView = other.m_iIndexForTableView;//记录在设备列表中的位置，第几行
    self.m_iIndexForImageView = other.m_iIndexForImageView;//记录在那个播放窗口显示
    ...
    self.m_iUserType = other.m_iUserType;//用户类型
