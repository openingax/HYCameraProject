...
#define MAX_CHANNEL_OF_BATTERY_CAM  4
...
typedef enum
{
	...
	SEP2P_MSG_GET_USER_INFO2_REQ	                   = 0x0144,
	SEP2P_MSG_GET_USER_INFO2_RESP		               = 0x0145,
	SEP2P_MSG_SET_USER_INFO2_REQ	                   = 0x0146,
	SEP2P_MSG_SET_USER_INFO2_RESP		               = 0x0147,
	//----}} X series P2P FW>=1.0.0.43------
	...
	SEP2P_MSG_FTP_GET_TEST_RESULT_REQ	               = 0x0164,
	SEP2P_MSG_FTP_GET_TEST_RESULT_RESP				   = 0x0165,
	...
	SEP2P_MSG_EMAIL_GET_TEST_RESULT_REQ	               = 0x0174,
	SEP2P_MSG_EMAIL_GET_TEST_RESULT_RESP               = 0x0175,
	...
	SEP2P_MSG_MANUAL_REC_ON_SD_REQ					   = 0x01B4,	//MP2P, 1Mpixel,2Mpixel, 2018-07-15
	SEP2P_MSG_MANUAL_REC_ON_SD_RESP		               = 0x01B5,	//MP2P, 1Mpixel,2Mpixel, 2018-07-15

	SEP2P_MSG_DEL_REMOTE_REC_FILE_REQ                  = 0x01B8,	//added on firmware v1.0.1.6
	SEP2P_MSG_DEL_REMOTE_REC_FILE_RESP                 = 0x01B9,	//added on firmware v1.0.1.6
	...
	SEP2P_MSG_RESTORE_FACTORY			= 0x01BE,	//X series, restore to default(factory)
	//
	
	//----{{ work together with upgraded server
	SEP2P_MSG_REMOTE_UPGRADE_REQ		= 0x01C0,	//added on v1.0.1.6
	SEP2P_MSG_REMOTE_UPGRADE_RESP		= 0x01C1,	//added on v1.0.1.6
	SEP2P_MSG_REMOTE_UPGRADE_CANCEL_REQ	= 0x01C2,	//added on v1.0.2.5, XP2P_mini FW>=2.0.1.40
	SEP2P_MSG_REMOTE_UPGRADE_CANCEL_RESP= 0x01C3,	//added on v1.0.2.5, XP2P_mini FW>=2.0.1.40
	SEP2P_MSG_REMOTE_UPGRADE_PROGRESS	= 0x01C5,	//added on v1.0.2.5, XP2P_mini FW>=2.0.1.40
	//----}} work together with upgraded server

	SEP2P_MSG_WRITE_FILE_TO_SDCARD_START	 = 0x01CA,
	SEP2P_MSG_WRITE_FILE_TO_SDCARD_START_RESP= 0x01CB,
	SEP2P_MSG_WRITE_FILE_TO_SDCARD_STOP		 = 0x01CC,
	SEP2P_MSG_WRITE_FILE_TO_SDCARD_STOP_RESP = 0x01CD,
	SEP2P_MSG_WRITE_FILE				= 0x01CE,
	SEP2P_MSG_WRITE_FILE_PROGRESS		= 0x01CF,

	SEP2P_MSG_DOWNLOAD_SD_START_REQ		= 0x01D0,
	SEP2P_MSG_DOWNLOAD_SD_START_RESP	= 0x01D1,
	SEP2P_MSG_DOWNLOAD_SD_STOP_REQ      = 0x01D2,
	SEP2P_MSG_DOWNLOAD_SD_STOP_RESP     = 0x01D3,

	SEP2P_MSG_BATTERY_CAM_SET_ALIAS_REQ		=0x01DE,
	SEP2P_MSG_BATTERY_CAM_SET_ALIAS_RESP	=0x01DF,
	SEP2P_MSG_BATTERY_CAM_PAIR_INFO_REQ		=0x01E0,
	SEP2P_MSG_BATTERY_CAM_PAIR_INFO_REPORT	=0x01E1,

	SEP2P_MSG_BATTERY_CAM_GET_PARAM_REQ	=0x01E2,
	SEP2P_MSG_BATTERY_CAM_GET_PARAM_RESP=0x01E3,
	SEP2P_MSG_BATTERY_CAM_SET_PARAM_REQ	=0x01E4,
	SEP2P_MSG_BATTERY_CAM_SET_PARAM_RESP=0x01E5,

	SEP2P_MSG_BATTERY_CAM_DEL_CHANNEL_REQ	=0x01E6,
	SEP2P_MSG_BATTERY_CAM_DEL_CHANNEL_RESP	=0x01E7
...
typedef enum 
{
	...
	//PTZ_CTRL_PRESET_BIT_CLEAR	=0x42,
		...
		//----{{MP2P, 1Mpixel,2Mpixel, 2018-07-15
	PTZ_CTRL_ZOOMIN		=0x60,	
	PTZ_CTRL_ZOOMOUT	=0x61,
	PTZ_CTRL_FOCUSIN	=0x62,
	PTZ_CTRL_FOCUSOUT	=0x63
		//----}}MP2P, 1Mpixel,2Mpixel, 2018-07-15
...
typedef enum
{
	...
	BIT_MASK_CAM_PARAM_FPS_KBPS	  =0x0200,
	BIT_MASK_CAM_PARAM_MIC_VOLUME =0x0400, //Microphone volume of IPC, 20161214
	BIT_MASK_CAM_PARAM_SPK_VOLUME =0x0800, //speaker volume of IPC,20161214
	BIT_MASK_CAM_PARAM_PIR_ENABLE =0x1000, //PIR enable of IPC,20170401
	BIT_MASK_CAM_PARAM_PIR_FLAG   =0x2000, //PIR flag of IPC,20170401
	BIT_MASK_CAM_PARAM_GOP		  =0x4000  //GOP of H264 20170620, XP2P FW>=1.0.1.31
...
typedef struct tag_MSG_GET_CAMERA_PARAM_RESP
{
	...
	INT32 bOSD;			//CHAR* pBytOSD=(CHAR *)&bOSD; 0->disable,1->enable; pBytOSD[0],time OSD; pBytOSD[1],name OSD; pBytOSD[2], Temperature&Humidity //No this item to L series;
	INT32 nMode;		//0->50Hz, 1->60Hz
	INT32 nFlip;		//0->normal, 1->flip, 2->mirror, 3->flip & mirror
	INT32 nKBPS;		//[1,x],  mask: BIT_MASK_CAM_PARAM_FPS_KBPS, MP2P FW>=v0.1.0.35, XP2P FW>=v1.0.1.7
	CHAR  nFPS;			//[1,60], mask: BIT_MASK_CAM_PARAM_FPS_KBPS, MP2P FW>=v0.1.0.35, XP2P FW>=v1.0.1.7
	CHAR  bPIREnable;	//PIR enable, 0->disable,1->enable; M series MP2P FW>=v0.1.0.41
	CHAR  bPIRFlag;		//PIR flag, 0->close,1->open; M series MP2P FW>=v0.1.0.41
	CHAR  nGop;			//XP2P FW>=1.0.1.31, gop value of h264 in current channel(resolution), its range is [2,150]
	CHAR  nIRLed;		//0->close, 1->open, 2->auto //support getting its value when FirmwareVer>=0.1.0.18 for only M Series
	CHAR  bHasMIC_SPKVolume; //0-->none, 1-->has mic spk volume function when get
	UCHAR nMICVolume;	//MIC volume, range is [1,100]
	UCHAR nSPKVolume;	//speaker volume, range is [1,100]
	CHAR  reserve3[12];
	CHAR  chOSDName[60]; //X series P2P FW>=1.0.0.43
...
typedef struct tag_MSG_SET_CAMERA_PARAM_REQ
{
	...
	INT32 reserve1;
	INT32 nSaturation;	//[0,255]
	INT32 bOSD;			//CHAR* pBytOSD=(CHAR *)&bOSD; 0->disable,1->enable;  pBytOSD[0],time OSD; pBytOSD[1],name OSD; pBytOSD[2], Temperature&Humidity //No these item to L series;
	INT32 nMode;		//0->50Hz,1->60Hz; NOTES:the device will reboot when setting for M series
	INT32 nFlip;		//0->normal, 1->flip, 2->mirror, 3->flip & mirror
	INT32 nKBPS;		//[1,x],  mask: BIT_MASK_CAM_PARAM_FPS_KBPS, MP2P FW>=v0.1.0.35, XP2P FW>=v1.0.0.40
	CHAR  nFPS;			//[1,60], mask: BIT_MASK_CAM_PARAM_FPS_KBPS, MP2P FW>=v0.1.0.35, XP2P FW>=v1.0.0.40
	CHAR  bPIREnable;	//PIR enable, 0->disable,1->enable; M series MP2P FW>=v0.1.0.41
	CHAR  bPIRFlag;		//PIR flag, 0->close,1->open; M series MP2P FW>=v0.1.0.41
	CHAR  nGop;			//XP2P FW>=1.0.1.31, gop value of h264 in current channel(resolution), its range is [2,150]
...
typedef struct tag_MSG_PTZ_CONTROL_REQ
{
	...
	UCHAR nCtrlParam;	// for PTZ_CTRL_PRESET_BIT_...: nCtrlParam is SET/GOTO/CLEAR preset bit no. range is [0,15]
						// for PTZ_CTRL_UP, PTZ_CTRL_DOWN, PTZ_CTRL_LEFT, PTZ_CTRL_RIGHT: nCtrlParam=1:continue after one step; 0=one step

	UCHAR nChannel;
	UCHAR nSpeed;		//MP2P, 1Mpixel,2Mpixel, 2018-07-15
	CHAR  reserve[12];
}MSG_PTZ_CONTROL_REQ;
...
typedef struct tag_MSG_GET_USER_INFO2_REQ
{
	CHAR  reserve[16];
}MSG_GET_USER_INFO2_REQ;

//SEP2P_MSG_GET_USER_INFO2_RESP
typedef enum tag_E_ROLE_ID
{
	ROLE_ID_ADMIN = 1,
	ROLE_ID_USER  =	2,
	ROLE_ID_GUEST = 3
}E_ROLE_ID;
typedef enum tag_E_OPCODE_USER_INFO
{
	OP_USER_INFO_ADD  = 1,
	OP_USER_INFO_DEL  =	2,
	OP_USER_INFO_EDIT = 3
...
typedef struct tag_USER_INFO{
	CHAR  chUsername[32]; //use it when add, edit, delete
	CHAR  chUserPwd[32];  //use it when add, edit
	INT32 nUserRoleID;	  //use it when add, edit, refer to E_ROLE_ID
	INT32 nUserID;		  //use it when edit, delete. can't delete the user of 'nUserID=10001'
}USER_INFO;
typedef struct tag_MSG_GET_USER_INFO2_RESP
{
	UCHAR nUserNum;
	UCHAR nCurUserRoleID; //current logined user role id
	CHAR  bCanAddDel;	  //=0 for L,M series, X p2p_fw<1.0.0.43;  =1,X p2p_fw>=1.0.0.43
	CHAR  reserve;
	INT32 nCurUserID;	  //L,M,X p2p_fw<1.0.0.43:  nUserID is 1(admin), 2(user), 3(guest)
	USER_INFO arrUserInfo[MAX_NUM_USER_INFO];
}MSG_GET_USER_INFO2_RESP, *PMSG_GET_USER_INFO2_RESP;

//SEP2P_MSG_SET_USER_INFO2_REQ
//the user of nUserID=10001 can NOT change role id and delete it!!
typedef struct tag_MSG_SET_USER_INFO2_REQ
{
	UCHAR nOpUserNum;//adding, editting, deleting user amount
	UCHAR nOpCode;	 //referto E_OPCODE_USER_INFO; 1:add; 2:delete; 3:edit;
	CHAR  reserve[6];
	USER_INFO arrUserInfo[MAX_NUM_USER_INFO];
}MSG_SET_USER_INFO2_REQ, *PMSG_SET_USER_INFO2_REQ;

//SEP2P_MSG_SET_USER_INFO2_RESP
typedef struct tag_MSG_SET_USER_INFO2_RESP
{
	CHAR  result;	//0: OK; or not fail, 1:unknown error, 2:wrong MSG_SET_USER_INFO2_REQ info, 3:permission denied
	UCHAR nOpUserNum;
	UCHAR nOpCode;  //=MSG_SET_USER_INFO2_REQ.nOpCode
	CHAR  reserve[13];
	INT32 nUserID[MAX_NUM_USER_INFO]; //editting or deleting USER_INFO.nUserID
}MSG_SET_USER_INFO2_RESP;
...
typedef struct tag_MSG_GET_FTP_INFO_RESP
{
	...
	//----{{2018-03-20
	CHAR bTestWhenSetting; //only use to set request; 0-->don't test; 1-->test;  X system firmware>=aa.2bbcc.x.18; it is valid for MSG_SET_FTP_INFO_REQ
	CHAR bHasTestFunction; //=0 for L,M series, X p2p_fw<1.a.b.c;  =1,for X system firmware>=aa.2bbcc.x.18; when bHasTestFunction=1, bTestWhenSetting=1 is valid
	//----}}2018-03-20
	CHAR reserve2[2];
...
//SEP2P_MSG_FTP_GET_TEST_RESULT_REQ	 = 0x0164,
//when MSG_SET_FTP_INFO_REQ.bTestWhenSetting=1 first send MSG_SET_FTP_INFO_REQ message then send this to query test result.
//-

//SEP2P_MSG_FTP_GET_TEST_RESULT_RESP = 0x0165,
typedef struct tag_MSG_FTP_GET_TEST_RESULT_RESP
{
	CHAR  result;	//0: OK; 1:unknown error; 2:connect error; 3:authroize error; 4:email testing
	CHAR  reserve[15];
}MSG_FTP_GET_TEST_RESULT_RESP;
...
typedef struct tag_MSG_GET_EMAIL_INFO_RESP
{
	...
	CHAR chSubject[64];
	INT32  nSMTPPort;
	INT32  nSSLAuth; //0=NONE, 1=SSL, 2=TLS;  XP2P(X series): 0=none, 1=ssl/tls, 2=starttls
	CHAR chText[64]; //add 2014-11-13, x series

	CHAR bTestWhenSetting; //only use to set request; 0-->don't test; 1-->test;  X series P2P FW>=1.0.0.43; it is valid for SEP2P_MSG_SET_EMAIL_INFO_REQ
	CHAR bHasTestFunction; //=0 for L,M series, X p2p_fw<1.0.0.43;  =1,X p2p_fw>=1.0.0.43;  when bHasTestFunction=1, bTestWhenSetting=1 is valid
	CHAR reserve[6];
...
{
	CHAR  result;	//0: OK; or not fail, 1: unknown error; 2: email testing
	CHAR  reserve[15];
}MSG_SET_EMAIL_INFO_RESP;

//SEP2P_MSG_EMAIL_GET_TEST_RESULT_REQ
//when MSG_SET_EMAIL_INFO_REQ.bTestWhenSetting=1 first send MSG_SET_EMAIL_INFO_REQ message then send this to query test result.
//-

//SEP2P_MSG_EMAIL_GET_TEST_RESULT_RESP
typedef struct tag_MSG_EMAIL_GET_TEST_RESULT_RESP
{
	CHAR  result;	//0: OK; 1:unknown error; 2:connect error; 3:authroize error; 4:email testing
	CHAR  reserve[15];
}MSG_EMAIL_GET_TEST_RESULT_RESP;
...
typedef struct tag_MSG_GET_ALARM_INFO_RESP
{
	...
	//the whole area will be setted when 4 area(md_x,md_y,md_width,md_height) are 0
	...
	//0=none,1=has temperature and humidity function; 
	//when it is 1 the field:bTemperatureAlarm,bTemperatureAlarm, nTempMinValueWhenAlarm, nTempMaxValueWhenAlarm,nHumiMinValueWhenAlarm, nHumiMaxValueWhenAlarm is valid;
	//for X P2P fw version>=1.0.0.49
	UCHAR bHasTempHumiFunction;
	UCHAR bNotSetAlarmTime; //only for setting 0:set alarm time; 1: not set
	UCHAR reserve3[3];
...
typedef struct tag_MSG_GET_SDCARD_REC_PARAM_RESP
{
	...
	INT32 nScheduleTime_sun_0;		//24 hours every day, 15 minutes every hour, one flag every 15 minutes, one flag is one bit. bit0---bit95, 0 is invalid in this period(15 minutes), 1 is valid in this period(15 minutes). 
	INT32 nScheduleTime_sun_1;		//nScheduleTime_sun_0.bit0=00:00:00--00:14:59; nAlarmTime_sun_0.bit1=00:15:00--00:29:59; nScheduleTime_sun_0.bit2=00:30:00--00:44:59; ......
	INT32 nScheduleTime_sun_2;		//nScheduleTime_sun_1.bit0=08:00:00--08:14:59; nScheduleTime_sun_1.bit1=08:15:00--08:29:59; nScheduleTime_sun_1.bit2=08:30:00--08:44:59; ......
	INT32 nScheduleTime_mon_0;		
	INT32 nScheduleTime_mon_1;		//nScheduleTime_sun_0, nScheduleTime_sun_1, nScheduleTime_sun_2: Sunday
	INT32 nScheduleTime_mon_2;		//nScheduleTime_mon_0, nScheduleTime_mon_1, nScheduleTime_mon_2: Monday
	INT32 nScheduleTime_tue_0;		//......
	INT32 nScheduleTime_tue_1;
	INT32 nScheduleTime_tue_2;
	INT32 nScheduleTime_wed_0;
	INT32 nScheduleTime_wed_1;
	INT32 nScheduleTime_wed_2;
	INT32 nScheduleTime_thu_0;
	INT32 nScheduleTime_thu_1;
	INT32 nScheduleTime_thu_2;
	INT32 nScheduleTime_fri_0;
	INT32 nScheduleTime_fri_1;
	INT32 nScheduleTime_fri_2;
	INT32 nScheduleTime_sat_0;
	INT32 nScheduleTime_sat_1;
	INT32 nScheduleTime_sat_2;
	INT32 nSDCardStatus;	//0: no sdcard; 1: sdcard inserted
	INT32 nSDTotalSpace;	//in MBytes
	INT32 nSDFreeSpace;		//in MBytes

							//----{{ X series P2P FW>=1.0.0.43------
	UCHAR nTotalChnRecordWhenGetting;	//only use to get message
	UCHAR nCurChnRecording;				//current channel to record, begin from 0
	CHAR  reserve3[6];
							//----}} X series P2P FW>=1.0.0.43------
...
typedef struct tag_MSG_GET_DEVICE_VERSION_RESP
{
	...
	CHAR  bSupportSpecialChar;//the camera site: 0: no, 1: yes; //2018-04-27
	CHAR  reserve1[2];
	CHAR  imn_server_port[96];//e.g.: domain_or_ip1:port1|domain_or_ip2:port2|...
							  //----{{ X series P2P FW>=1.0.0.43------
	UCHAR  chLANIP[4];		  //ipv4, LAN IP address of the device
	UCHAR  chWANIP[4];		  //ipv4, WAN IP address of the device
	UINT16 nHttpPort;		  //http port of the device
	UCHAR  chMACAddress[6];	  //using MAC address of the device
							  //----}} X series P2P FW>=1.0.0.43------
	CHAR  nPartitionNum;	  //right value=3 or 5, 0 is unknown //XP2P FW>=1.0.1.36
	CHAR  bUpgradeByApp;	  //0=disable; 1=enable. whether or not support upgrade by the mobile app
	CHAR  nModeOfDevice;	  //0=normal mode; 1=upgrade mode;   //XP2P FW>=1.0.1.39, XP2P_mini FW>=2.0.1.39
	CHAR  reserve[1];
	CHAR  chFwddns_web_ver[28]; //sys web version
	CHAR  chSn[32];			 //factory serial number,for v2.0 ARCH IPC added on 2018-04-01
}MSG_GET_DEVICE_VERSION_RESP, *PMSG_GET_DEVICE_VERSION_RESP; //280 bytes
...
//SEP2P_MSG_MANUAL_REC_ON_SD_REQ					= 0x01B4,
typedef struct tag_MSG_MANUAL_REC_ON_SD_REQ
{
	UCHAR bOn;
	CHAR reserve[15];
}MSG_MANUAL_REC_ON_SD_REQ;
//SEP2P_MSG_MANUAL_REC_ON_SD_RESP		            = 0x01B5,
typedef struct tag_SEP2P_MSG_MANUAL_REC_ON_SD_RESP
{
	UCHAR bOn;
	UCHAR nResult;		//0:OK; 1:failed
	CHAR reserve[14];
}MSG_MANUAL_REC_ON_SD_RESP;


//SEP2P_MSG_DEL_REMOTE_REC_FILE_REQ                = 0x01B8,	//added on v1.0.1.6
typedef struct tag_MSG_DEL_REMOTE_REC_FILE_REQ
{
	CHAR  nOperCode; //0:del one file 
	CHAR  reserve[15];
	CHAR  chFilePath[120];
}MSG_DEL_REMOTE_REC_FILE_REQ;
//SEP2P_MSG_DEL_REMOTE_REC_FILE_RESP               = 0x01B9,	//added on v1.0.1.6
typedef struct tag_MSG_DEL_REMOTE_REC_FILE_RESP
{
	CHAR  nResult;		//0:OK; 1:failed
	CHAR  reserve[15];
	CHAR  chFilePath[120];
}MSG_DEL_REMOTE_REC_FILE_RESP;
...
//SEP2P_MSG_REMOTE_UPGRADE_REQ		= 0x01C0,
typedef struct tag_MSG_REMOTE_UPGRADE_REQ
{
	CHAR chServIP[64];
	UINT16 nServPort;
	CHAR reserve1[6];
	CHAR chServUsername[32];
	CHAR chServPasswd[32];

	CHAR chWillUpg_SysFWVersion[24];
	CHAR chWillUpg_SysFWBuiltDate[24];
	CHAR chWillUpg_CrcValue[32]; //hex string
	CHAR chWillUpg_FWPathOnServ[128];
	INT32 nFileSize;
	CHAR reserve2[124];
}MSG_REMOTE_UPGRADE_REQ;


//SEP2P_MSG_REMOTE_UPGRADE_RESP		= 0x01C1
typedef struct tag_MSG_REMOTE_UPGRADE_RESP
{
	//nResult:
	//	0:ok
	//	1:unknown error
	//	2:failed to get info from system firmware
	//	3:system firmware that will upgrade mismatch with itself
	//	4:failed to connect ServIP & ServPort
	//	5:wrong chServUsername or chServPasswd
	//	6:remote upgrade of this device is NOT supported
	CHAR nResult;
	CHAR reserve[7];
	CHAR chWillUpg_SysFWVersion[24];
}MSG_REMOTE_UPGRADE_RESP;

//SEP2P_MSG_REMOTE_UPGRADE_CANCEL_REQ	= 0x01C2,	//added on v1.0.2.5, XP2P_mini FW>=2.0.1.40
typedef struct tag_MSG_REMOTE_UPGRADE_CANCEL_REQ
{
	CHAR reserve[8];
}MSG_REMOTE_UPGRADE_CANCEL_REQ;

//SEP2P_MSG_REMOTE_UPGRADE_CANCEL_RESP= 0x01C3,	//added on v1.0.2.5, XP2P_mini FW>=2.0.1.40
typedef struct tag_MSG_REMOTE_UPGRADE_CANCEL_RESP
{
	//nResult:
	//	0:ok
	CHAR nResult; //1=download ok, upgrading, so don't cancel
	CHAR reserve[7];
}MSG_REMOTE_UPGRADE_CANCEL_RESP;


//SEP2P_MSG_REMOTE_UPGRADE_PROGRESS	= 0x01C5
typedef enum tag_E_UPG_STATUS{
	UPG_STATUS_FW_READY     	=0,
	UPG_STATUS_FW_DOWNLOADING	=1,
	UPG_STATUS_FW_DOWNLOAD_OK	=2,
	UPG_STATUS_FW_DOWNLOAD_FAIL	=3,
	UPG_STATUS_FW_ERASING		=4,
	UPG_STATUS_FW_ERASED		=5,
	UPG_STATUS_FW_REBOOTING		=6
}E_UPG_STATUS;
typedef struct tag_MSG_REMOTE_UPGRADE_PROGRESS
{
	CHAR chDID[24];
	CHAR chWillUpg_SysFWVersion[24];
	E_UPG_STATUS nStatus; 
	INT32 nProgress; //Percentage, e.g.: nProgress=40, that is 40%
}MSG_REMOTE_UPGRADE_PROGRESS;


//SEP2P_MSG_WRITE_FILE_TO_SDCARD_START	= 0x01CA,
typedef struct tag_MSG_WRITE_FILE_TO_SDCARD_START
{
	INT32 nTotalSize;
	INT32 nFileID;		//user defined, e.g.file no: 0,1,2,...
	CHAR  nWriteMode;	//0:add, 1:delete
	CHAR  bUpgradeFW;   //temp upgrade fw for battery cam
	CHAR  reserve[6];
	CHAR  chSaveAsNameWithPath[256]; //filename at the camera site, "/" is root dir
}MSG_WRITE_FILE_TO_SDCARD_START;

//SEP2P_MSG_WRITE_FILE_TO_SDCARD_START_RESP	= 0x01CB,
typedef struct tag_MSG_WRITE_FILE_TO_SDCARD_START_RESP
{
	INT32 nTotalSize;
	INT32 nFileID;		//user defined, e.g.file no: 0,1,2,...
	//nResult:
	//	0:ok
	//	1:unknown error
	//	2:busy, writing data to sdcard, please wait
	//	3:no sd/tf card
	//	4:failed to create path
	//	5:failed when writing data to sd/tf card
	CHAR  nResult;
	CHAR  nWriteMode; //0:add, 1:delete
	CHAR  reserve[6];
}MSG_WRITE_FILE_TO_SDCARD_START_RESP;

//SEP2P_MSG_WRITE_FILE_TO_SDCARD_STOP	= 0x01CC,
typedef struct tag_MSG_WRITE_FILE_TO_SDCARD_STOP
{
	INT32 nFileID; //user defined, e.g.file no: 0,1,2,..., MSG_WRITE_FILE_TO_SDCARD_START.nFileID
	CHAR  chReserve[4];
}MSG_WRITE_FILE_TO_SDCARD_STOP;

//SEP2P_MSG_WRITE_FILE_TO_SDCARD_STOP_RESP	= 0x01CD,
typedef struct tag_MSG_WRITE_FILE_TO_SDCARD_STOP_RESP
{
	INT32 nFileID; //user defined, e.g.file no: 0,1,2,...
	//nResult:
	//	0:ok
	...
	CHAR  nResult;
	CHAR  reserve[3];
}MSG_WRITE_FILE_TO_SDCARD_STOP_RESP;


//SEP2P_MSG_WRITE_FILE	= 0x01CE,
typedef struct tag_MSG_WRITE_FILE
{
	INT32 nFileID; //user defined, e.g.file no: 0,1,2,...
	INT32 nSliceNo_1KBytes;		//begin from 0. e.g.: 0,1,2,...
	INT32 nSliceSize;	//Unit: bytes
	CHAR  reserve[4];
	//CHAR  chData[0];
}MSG_WRITE_FILE;

//SEP2P_MSG_WRITE_FILE_PROGRESS	= 0x01CF,
//the camera reply this packet at the first, the last packet and every receiving the 40KBytes data
typedef struct tag_MSG_WRITE_FILE_PROGRESS
{
	INT32 nFileID; //user defined, e.g.file no: 0,1,2,...
	INT32 nSliceNo_1KBytes;		//begin from 0. e.g.: 0,1,2,...
	INT32 nSliceSize;	//Unit: bytes, <=1024
	//nResult:
	//	0:ok
	//	1:unknown error
	//	2:No this nFileID
	...
	CHAR  nResult;
	CHAR  reserve[3];
}MSG_WRITE_FILE_PROGRESS;

//get the record files of sd by SEP2P_MSG_GET_REMOTE_REC_... message below
//SEP2P_MSG_GET_REMOTE_REC_DAY_BY_MONTH_REQ
//SEP2P_MSG_GET_REMOTE_REC_DAY_BY_MONTH_RESP
//SEP2P_MSG_GET_REMOTE_REC_FILE_BY_DAY_REQ
//SEP2P_MSG_GET_REMOTE_REC_FILE_BY_DAY_RESP

//SEP2P_MSG_DOWNLOAD_SD_START_REQ			= 0x01D0,
typedef struct tag_MSG_DOWNLOAD_SD_START_REQ
{
	CHAR chFilePath[120]; //will download the record filename with full path
	CHAR reserve[8];
}MSG_DOWNLOAD_SD_START_REQ;

//SEP2P_MSG_DOWNLOAD_SD_START_RESP			= 0x01D1,
typedef struct tag_MSG_DOWNLOAD_SD_START_RESP
{
	CHAR chFilePath[120];
	//nResult:
	//	0:ok
	...
	CHAR  nResult;
	CHAR  reserve[3];
	UINT32 nFileIDWillDownload;
}MSG_DOWNLOAD_SD_START_RESP;

//downloading file data will receive 
//from POnStreamCallback of SEP2P_SetStreamCallback(const CHAR* pDID, POnStreamCallback pStreamCallback, VOID *pUserData)

//SEP2P_MSG_DOWNLOAD_SD_STOP_REQ      = 0x01D2,
typedef struct tag_MSG_DOWNLOAD_SD_STOP_REQ
{
	UINT32 nFileIDWillDownload;
	CHAR reserve[4];
}MSG_DOWNLOAD_SD_STOP_REQ;

//SEP2P_MSG_DOWNLOAD_SD_STOP_RESP     = 0x01D3
typedef struct tag_MSG_DOWNLOAD_SD_STOP_RESP
{
	UINT32 nFileIDWillDownload;
	//nResult:
	//	0:ok
	//	1:unknown error
	//	2:download id don't exist
	CHAR  nResult;
	CHAR  reserve[3];
}MSG_DOWNLOAD_SD_STOP_RESP;


//SEP2P_MSG_BATTERY_CAM_SET_ALIAS_REQ		=0x01DE
typedef struct tag_MSG_BATTERY_CAM_SET_ALIAS_REQ
{
	UCHAR  nChannelCam;	//camera index, begin from 0
	CHAR   reserve[7];
	CHAR   chAlias[24]; //camera name
}MSG_BATTERY_CAM_SET_ALIAS_REQ;

//SEP2P_MSG_BATTERY_CAM_SET_ALIAS_RESP	=0x01DF
typedef struct tag_MSG_BATTERY_CAM_SET_ALIAS_RESP
{
	UCHAR  nChannelCam;
	UCHAR  nResult;		//0:ok, other:fail
	CHAR   reserve[6];
	CHAR  chAlias[24];  //camera name
}MSG_BATTERY_CAM_SET_ALIAS_RESP;

//#define SEP2P_MSG_BATTERY_CAM_PAIR_INFO_REQ 0x01E0
// -
//#define SEP2P_MSG_BATTERY_CAM_PAIR_INFO_REPORT 0x01E1
typedef struct tagCAM_PAIR_INFO{
	CHAR  chCamMACAddr[18];	//mac address
	CHAR  chCamIP[16];		//ip address
	CHAR  nCamIndex;		//camera index no. pair with base station
	CHAR  bCamPaired;		//0:no pair, 1:paired 
	CHAR  bCamOnline;		//0:no online;1:online 
	UCHAR CAM_nPower;		//[0,5] //0: low power, 4: full power, 5: usb power
	CHAR  reserve[2];

	CHAR  chVerMcu[24]; //camera site MCU version
	CHAR  chVerDsp[8];  //camera site DSP version
	CHAR  chAlias[24];  //camera name
	CHAR  reserve2[24];
}CAM_PAIR_INFO;
typedef struct tag_MSG_BATTERY_CAM_PAIR_INFO_REPORT //to recompile lib if this struct is changed.
{
	CHAR  reserve[3];
	UCHAR nPairNum;
	UCHAR nPairIndex[MAX_CHANNEL_OF_BATTERY_CAM]; //e.g. nPairNum=2 (camera index 1,camera index 3 is paired. 0xFF is NOT paired. )  then nPairIndex[0]=1, nPairIndex[1]=3
	CAM_PAIR_INFO stPairInfo[MAX_CHANNEL_OF_BATTERY_CAM];
}MSG_BATTERY_CAM_PAIR_INFO_REPORT;

//#define SEP2P_MSG_BATTERY_CAM_GET_PARAM_REQ 0x01E2
typedef struct tag_MSG_BATTERY_CAM_GET_PARAM_REQ
{
	UCHAR  nChannelCam;				//camera index, begin from 0
	CHAR   reserve2[7];
}MSG_BATTERY_CAM_GET_PARAM_REQ;

//#define SEP2P_MSG_BATTERY_CAM_GET_PARAM_RESP 0x01E3
typedef enum{
	CAM_VIDEO_OSD_NO=0,				//0=no show
	CAM_VIDEO_OSD_ONLYTIME,			//1=show time
}ENUM_BATTERY_CAM_VIDEO_OSD;
typedef enum
{
	BIT_MASK_VIDEO_OSD  =0x01, //set: CAM_VIDEO_OSD_eValue
	BIT_MASK_VIDEO_FLIP =0x02,
	BIT_MASK_PIR	    =0x04, //set: bPIR, nPIRSensitivity
	BIT_MASK_REC		=0x08, //set: BS_REC_nRecLenOfSD_sec
	BIT_MASK_BELL		=0x10, //set: BS_BELL_bBell, BS_BELL_nBellVolume

	BIT_MASK_BEEP		=0x20, //set: bBeepWhenBoot
	BIT_MASK_LED		=0x40, //set: bLEDRedAfterBoot, bLEDBlueAfterBoot
	BIT_MASK_PUSH		=0x80, //set: BS_bPushWhenAlarm
	BIT_MASK_LANGUAGE	=0x100,//set: BS_nIndexLanguageOfVoice
	BIT_MASK_SERV_PUSH	=0x200 //set: BS_bServPush
}ENUM_BIT_MASK_BATTERY_CAM;
typedef enum
{
	LANG_VOICE_EN	=0,
	LANG_VOICE_CN_mandarin,
	LANG_VOICE_DE,		//German
	LANG_VOICE_FR,		//French

}ENUM_BS_LANGUAGE_VOICE;

typedef struct tag_MSG_BATTERY_CAM_GET_PARAM_RESP
{
	UCHAR  nChannelCam;				//camera index, begin from 0
	UCHAR  CAM_nPower;		//only get; [0,5] //0: low power, 4: full power, 5: usb power
	UCHAR  CAM_VIDEO_nFlip;	//0->normal, 1->flip, 2->mirror, 3->flip & mirror
	UCHAR  CAM_PIR_nPIRSensitivity; //[0, 100], 100 is the most sensitive

	UCHAR  CAM_VIDEO_OSD_eValue;   //refer to ENUM_BATTERY_CAM_VIDEO_OSD
	UCHAR  BS_BELL_bBell;		   //0: base station don't bell when pressing bell key of camera; 1:bell
	UCHAR  BS_BELL_nBellVolume;	   //bell volume: [0,6]
	UCHAR  BS_BEEP_bBeepWhenBoot;

	UCHAR  BS_LED_bLEDRedAfterBoot;	//0:close, 1:open
	UCHAR  BS_LED_bLEDBlueAfterBoot;//0:close, 1:open
	UINT16 BS_REC_nRecLenOfSD_sec;	//[10s,30s], default: 15s
	
	UCHAR  BS_bPushWhenAlarm;
	UCHAR  BS_nIndexLanguageOfVoice;//refer to ENUM_BS_LANGUAGE_VOICE
	UCHAR  BS_nSDCardStatus;	//0: no sdcard; 1: sdcard inserted.  only get from camera
	UCHAR  BS_bServPush;		//0: disable push message by the server, 1:enable push message by the server

	UINT32  BS_nSDTotalSpace;	//in MBytes  only get from camera
	UINT32  BS_nSDFreeSpace;	//in MBytes  only get from camera

	CHAR   reserve2[12];
}MSG_BATTERY_CAM_GET_PARAM_RESP;


//#define SEP2P_MSG_BATTERY_CAM_SET_PARAM_REQ 0x01E4
typedef struct tag_MSG_BATTERY_CAM_SET_PARAM_REQ
{
	UCHAR  nChannelCam;		//camera index, begin from 0
	UCHAR  CAM_nPower;		//only get; [0,5] //0: low power, 4: full power, 5: usb power
	UCHAR  CAM_VIDEO_nFlip;	//0->normal, 1->flip, 2->mirror, 3->flip & mirror
	UCHAR  CAM_PIR_nPIRSensitivity; //[0, 100], 0 is disable. 1 is the min sensitive, 100 is the most sensitive

	UCHAR  CAM_VIDEO_OSD_eValue;   //refer to ENUM_BATTERY_CAM_VIDEO_OSD
	UCHAR  BS_BELL_bBell;		   //0: base station don't bell when pressing bell key of camera; 1:bell
	UCHAR  BS_BELL_nBellVolume;	   //bell volume: [0,6]; 0 is the mini volume, 6 is the max volume
	UCHAR  BS_BEEP_bBeepWhenBoot;
	...
	ENUM_BIT_MASK_BATTERY_CAM nBitMaskToSet; //refer to ENUM_BIT_MASK_BATTERY_CAM
	UCHAR  BS_bPushWhenAlarm;
	UCHAR  BS_nIndexLanguageOfVoice;	//refer to ENUM_BS_LANGUAGE_VOICE
	UCHAR  BS_bServPush;		//0: disable push message by the server, 1:enable push message by the server
	CHAR   reserve[5];
}MSG_BATTERY_CAM_SET_PARAM_REQ;

//#define SEP2P_MSG_BATTERY_CAM_SET_PARAM_RESP 0x01E5
typedef struct tag_MSG_BATTERY_CAM_SET_PARAM_RESP
{
	UCHAR  nChannelCam;				//camera index, begin from 0
	UCHAR  CAM_nPower;		//only get; [0,5] //0: low power, 4: full power, 5: usb power
	UCHAR  CAM_VIDEO_nFlip;	//0->normal, 1->flip, 2->mirror, 3->flip & mirror
	...
	UCHAR  BS_bServPush;		//0: disable push message by the server, 1:enable push message by the server
	CHAR   reserve[4];
}MSG_BATTERY_CAM_SET_PARAM_RESP;


//#define SEP2P_MSG_BATTERY_CAM_DEL_CHANNEL_REQ	0x01E6
typedef struct tag_MSG_BATTERY_CAM_DEL_CHANNEL_REQ
{
	UCHAR nChannelCam;		//camera index, begin from 0; 0<= nChannelCam < AV_PARAMETER.nMaxChannel, by api SEP2P_GetAVParameterSupported(.,AV_PARAMETER);
	CHAR  reserve[7];
}MSG_BATTERY_CAM_DEL_CHANNEL_REQ;

//#define SEP2P_MSG_BATTERY_CAM_DEL_CHANNEL_RESP 0x01E7
typedef struct tag_MSG_BATTERY_CAM_DEL_CHANNEL_RESP
{
	UCHAR nChannelCam;		//camera index, begin from 0; 0<= nChannelCam <AV_PARAMETER.nMaxChannel, by api SEP2P_GetAVParameterSupported(.,AV_PARAMETER);
	UCHAR nResult;			//0:ok, 1:unknown error, 2:no pair with base station
	CHAR  reserve[6];
}MSG_BATTERY_CAM_DEL_CHANNEL_RESP;
