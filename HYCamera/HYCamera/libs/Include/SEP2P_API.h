...
extern "C" {
	...
	typedef enum{
		DEVICE_TYPE_IPC =0,
		DEVICE_TYPE_NVR =1,
		DEVICE_TYPE_DOORBELL=2,
		DEVICE_TYPE_BATTERY_CAM=3 //added on 20180322
	}E_DEVICE_TYPE;
	...
	typedef struct tag_stAVParameter
	{
		...
		UCHAR  nDeviceType;		  // added on 20150504, 0=DEVICE_TYPE_IPC, 1=DEVICE_TYPE_NVR, 2=DEVICE_TYPE_DOORBELL, 3=DEVICE_TYPE_BATTERY_CAM, ...
		UCHAR  nMaxChannel;		  // nMaxChannel is how many cameras when nDeviceType=DEVICE_TYPE_NVR or DEVICE_TYPE_BATTERY_CAM 
		CHAR   reserve[6];
	}AV_PARAMETER;
    ...
    /*!
	 ...
	 when nLivePlayback=2 
		STREAM_HEAD.nChannel		(the data of some channel)
		STREAM_HEAD.nTimestamp		(total data size of this file) 
		STREAM_HEAD.nStreamDataLen	(data size of this packet)
		STREAM_HEAD.nPlaybackID		(fileID of this file)
		are valid, other fields are invalid
     */
	typedef struct tag_stStreamHead
	{
		...
		CHAR   nLivePlayback;// Video: 0:live video or audio;  1:playback video or audio;  2:file raw data downloading
		...
		UINT16 nTimestamp0;		//int64 timestamp=nTimestamp0<<32 + nTimestamp
								//	UINT64 nCurTS=pStreamHead->nTimestamp0;
								//	nCurTS=nCurTS<<32;
								//	nCurTS+=pStreamHead->nTimestamp;
	...
	typedef enum
	{
		...
		EVENT_TYPE_PIR_ALARM		 = 0x40,  //battery alarm: 红外报警
		EVENT_TYPE_TAMPER_ALARM		 = 0x80,  //battery alarm: 防拆报警消息	
		EVENT_TYPE_LOW_POWER_ALARM	 = 0x100, //battery alarm: 低电量报警
		EVENT_TYPE_USB_POWER_ON		 = 0x200, //battery alarm: 外部电源接入
		EVENT_TYPE_USB_POWER_OFF	 = 0x400  //battery alarm: 外部电源移除	
...
/*!
	...
	SEP2P_API_API INT32 SEP2P_SetAPIEnv(STAPIEnv *pStAPIEnv);
	...
	SEP2P_API_API INT32 SEP2P_Connect1(const CHAR* pDID, const CHAR* pUser, const CHAR* pPwd, INT32 nFlag);
